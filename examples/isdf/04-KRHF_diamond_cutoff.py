...
C_ARRAY = [25]  ## if rela_cutoff_QRCP is set, then c is used to when performing random projection, which can be relative large.
RELA_QR = [1e-3]
SuperCell_ARRAY = [
    [1,1,8],
]
        ...
        for basis in Basis:
            for ke_cutoff in Ke_CUTOFF:
                    ...
                    for c,rela_qr in list(zip(C_ARRAY,RELA_QR)):
                        ...
                        pbc_isdf_info.set_build_K_cutoff(rela_cutoff=1e-5, abs_cutoff=1e-6)      # rela_cutoff is useless currently
