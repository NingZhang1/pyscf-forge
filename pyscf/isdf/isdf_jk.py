...
def _contract_j_dm(mydf, dm, with_robust_fitting=True, use_mpi=False):
    ...
    buffer4 = np.ndarray(
        ...
        dtype=dm.dtype,
        buffer=buffer,
        offset=(nao * ngrid + ngrid + nao * naux) * dm.dtype.itemsize,
    )
    buffer5 = np.ndarray(
        (naux),
        dtype=dm.dtype,
        buffer=buffer,
        offset=(nao * ngrid + ngrid + nao * naux + naux) * dm.dtype.itemsize,
    )
    buffer6 = np.ndarray(
        (nao, nao),
        dtype=dm.dtype,
        buffer=buffer,
        offset=(nao * ngrid + ngrid + nao * naux + naux + naux) * dm.dtype.itemsize,
    )
    buffer7 = np.ndarray((nao, naux), dtype=dm.dtype, buffer=buffer, offset=0)
    buffer8 = np.ndarray(
        (naux), dtype=dm.dtype, buffer=buffer, offset=nao * ngrid * dm.dtype.itemsize
    )
    ...
    density_Rg = np.asarray(
        lib.multiply_sum_isdf(aoRg, tmp1, out=buffer4), order="C"
    )  # need allocate memory, size = naux, (buffer 4)
    ...
    if with_robust_fitting:
        J = np.asarray(
            lib.ddot(V_R, density_R.reshape(-1, 1), c=buffer5.reshape(-1, 1)), order="C"
        ).reshape(
            -1
        )  # with buffer, size
        ...
        J2 = np.asarray(
            lib.dot(V_R.T, density_Rg.reshape(-1, 1), c=buffer2.reshape(-1, 1)),
            order="C",
        ).reshape(-1)
    ...
    tmp = np.asarray(
        lib.dot(W, density_Rg.reshape(-1, 1), c=buffer8.reshape(-1, 1)), order="C"
    ).reshape(-1)
...
def _contract_k_dm(mydf, dm, with_robust_fitting=True, use_mpi=False):
    ...
    buffer4 = np.ndarray(
        ...
        dtype=dm.dtype,
        buffer=buffer,
        offset=(nao * ngrid + naux * ngrid + naux * naux) * dm.dtype.itemsize,
    )
...
def _contract_k_dm_wo_robust_fitting(
...
def get_jk_dm(
    ...
    **kwargs
    ...
    for iset in range(nset):
        ...
        if with_j:
            vj[iset] = _contract_j_dm_fast(
                mydf, dm[iset], mydf.with_robust_fitting, use_mpi
            )
        if with_k:
            if mydf.with_robust_fitting:
                vk[iset] = _contract_k_dm(
                    mydf, dm[iset], mydf.with_robust_fitting, use_mpi
                )
            else:
                vk[iset] = _contract_k_dm_wo_robust_fitting(
                    mydf, dm[iset], mydf.with_robust_fitting, use_mpi
                )
            if exxdiv == "ewald":
