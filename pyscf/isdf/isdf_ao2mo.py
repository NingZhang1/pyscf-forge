...
def isdf_eri(mydf, mo_coeff=None, verbose=None):
    ...
    if with_robust_fitting:
        ...
        max_nao_involved = np.max(
            [aoR_holder.aoR.shape[0] for aoR_holder in moRg if aoR_holder is not None]
        )
        max_ngrid_involved = None
    max_nIP_involved = np.max(
        [aoR_holder.aoR.shape[1] for aoR_holder in moRg if aoR_holder is not None]
    )
    ...
    if with_robust_fitting:
        ...
        for partition_i in range(natm):
            ...
            aoPair_i = np.ndarray(
                (nPair_i, nIP_i), dtype=np.float64, buffer=aoPairRg_buf
            )
            ...
            for partition_j in range(natm):
                ...
                aoPair_j = np.ndarray(
                    (nPair_j, ngrid_j), dtype=np.float64, buffer=aoPairR_buf
                )
                ...
                V_packed = np.ndarray(
                    (nIP_i, ngrid_j), dtype=np.float64, buffer=V_W_pack_buf
                )
                ...
                fn_packcol(
                    ...
                    ctypes.c_int(ngrid_j),
                    V_R[
                        global_IP_begin_i : global_IP_begin_i + nIP_i, :
                ...
                ddot_res = np.ndarray(
                    (nIP_i, nPair_j), dtype=np.float64, buffer=ddot_res_buf
                )
                lib.ddot(V_packed, aoPair_j.T, c=ddot_res)
                sub_eri = np.ndarray(
                    (nPair_i, nPair_j), dtype=np.float64, buffer=suberi_buf
                )
    ...
    for partition_i in range(natm):
        ...
        for partition_j in range(partition_i + 1):
            ...
            nPair_j = (nao_j * (nao_j + 1)) // 2
            aoPair_j = np.ndarray(
                (nPair_j, nIP_j), dtype=np.float64, buffer=aoPairRg_buf2
            )
            ...
            fn_packcol(
                ...
                ctypes.c_int(nIP_i),
                ctypes.c_int(nIP_j),
                W[global_IP_begin_i : global_IP_begin_i + nIP_i, :].ctypes.data_as(
                    ctypes.c_void_p
                ),
                ctypes.c_int(nIP_i),
...
def isdf_eri_2(mydf, mo_coeff=None, verbose=None):
    ...
    for partition_i in range(natm):
        ...
        if with_robust_fitting:
            ...
            assert nPair_i == npair
            aoPair_i = np.ndarray(
                (nPair_i, ngrid_i), dtype=np.float64, buffer=aoPairR_buf
            )
...
def isdf_eri_ovov(
    mydf, mo_coeff_o: np.ndarray = None, mo_coeff_v: np.ndarray = None, verbose=None
):
    """
    ...
    max_nIP_involved = np.max(
        [aoR_holder.aoR.shape[1] for aoR_holder in moRg_o if aoR_holder is not None]
    )
...
def get_eri(
    mydf, kpts=None, compact=getattr(__config__, "pbc_df_ao2mo_get_eri_compact", True)
):
...
def general(
    ...
    if not _iskconserv(cell, kptijkl):
        lib.logger.warn(
            cell,
            "fft_ao2mo: momentum conservation not found in " "the given k-points %s",
            kptijkl,
        )
...
def LS_THC_moeri(mydf, Z: np.ndarray, R: np.ndarray, mo_coeff: np.ndarray):
    ...
    path_info = np.einsum_path(
        einsum_str, moRg[0], moRg[1], Z, moRg[2], moRg[3], optimize="optimal"
    )
    res = np.einsum(
        einsum_str, moRg[0], moRg[1], Z, moRg[2], moRg[3], optimize=path_info[0]
    )
    log = lib.logger.Logger(mydf.stdout, mydf.verbose)
