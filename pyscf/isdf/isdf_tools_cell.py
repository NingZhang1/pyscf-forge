...
def build_primitive_cell(supercell: Cell, kmesh):
    ...
    prim_a = np.array(
        [
            supercell.a[0] / kmesh[0],
            supercell.a[1] / kmesh[1],
            supercell.a[2] / kmesh[2],
        ],
        dtype=np.float64,
    )
...
def build_supercell_with_partition(
    ...
    cell = build_supercell(
        ...
        precision=precision,
        use_particle_mesh_ewald=use_particle_mesh_ewald,
        verbose=verbose,
    )
        ...
        for iy in range(Ls[1]):
            for iz in range(Ls[2]):
                ...
                for sub_partition in partition:
                    partition_supercell.append(
                        [x + cell_id * natm_prim for x in sub_partition]
                    )
