...
from pyscf.isdf.BackEnd.isdf_backend import COMPLEX128 as COMPLEX128Ty
...
def ISDF_eval_gto(
    ...
    if out is None:
        # out = numpy.empty(
        #     (nkpts, comp, nao, ngrids), dtype=numpy.complex128
        # )  # NOTE THE definition of the shape!
        out = ZEROS((nkpts, comp, nao, ngrids), dtype=COMPLEX128Ty, cpu=True)
        out = ToNUMPY(out)
    else:
        # print("out is given")
        out = numpy.ndarray(
            (nkpts, comp, nao, ngrids), dtype=numpy.complex128, buffer=out
        )
