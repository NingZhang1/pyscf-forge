                ...
                for ix_col in range(Ls[0]):
                    for iy_col in range(Ls[1]):
                        for iz_col in range(Ls[2]):
                            ...
                            output[b_begin:b_end, k_begin:k_end] = input_mat[
                                :, k_begin2:k_end2
                            ]
            ...
            for iz in range(kmesh[2] // 2 + 1):
                ...
                if loc1 == loc2:
                    ...
                    if np.max(np.abs(imag_part)) > 1e-8:
                        print(
                            "Warning: max abs of imag_part = ",
                            np.max(np.abs(imag_part)),
                        )
                else:
                    output[loc1] = input_mat[:, loc * nao_prim : (loc + 1) * nao_prim]
                    output[loc2] = input_mat[
                        :, loc * nao_prim : (loc + 1) * nao_prim
                    ].conj()
                loc += 1
