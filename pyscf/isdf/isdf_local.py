...
def select_IP_atm_ls(
    ...
    group_begin, group_end = ISDF_Local_Utils._range_partition(
        first_natm, rank, comm_size, use_mpi
    )
    ...
    for atm_id in range(group_begin, group_end):
        ...
        fn_ik_jk_ijk(
            aoR_atm1.ctypes.data_as(ctypes.c_void_p),
        ...
        thread_buffer = np.ndarray(
            (nthread + 1, grid_ID.shape[0] + 1), dtype=np.float64
        )
        ...
        if USE_SCIPY_QR:
            ...
            log.debug4(
                "ngrid = %d, npt_find = %d, cutoff = %12.6e"
                % (grid_ID.shape[0], npt_find, cutoff)
            )
...
def select_IP_group_ls(
    mydf, aoRg_possible, c: int, m: int, group=None, atm_2_IP_possible=None
):
    ...
    log.debug4(
        "In select_IP_group_ls, naux2_now         = %d, max_rank = %d"
        % (naux2_now, max_rank)
    )
    ...
    thread_buffer = np.ndarray(
        (nthread + 1, IP_possible.shape[0] + 1), dtype=np.float64
    )
    ...
    if not USE_SCIPY_QR:
        ...
        log.debug4(
            "ngrid = %d, npt_find = %d, cutoff = %12.6e"
            % (IP_possible.shape[0], npt_find, cutoff)
        )
    else:
        ...
        R, pivot = scipy.linalg.qr(
            aoPairBuffer, pivoting=True, mode="r", check_finite=False, overwrite_a=True
        )
        npt_find = nao_group * c
...
def build_aux_basis_ls(mydf, group, IP_group, debug=True, use_mpi=False):
    ...
    for i in range(group_begin, group_end):
        ...
        if aoRg1.aoR.shape[0] == aoR1.aoR.shape[0]:
            ...
            pos1, pos2 = find_common_elements_positions(
        ...
        if not USE_SCIPY_CHOLESKY:
            ...
            C = scipy.linalg.cholesky(
                A, lower=True, overwrite_a=True, check_finite=False
            )
            B = scipy.linalg.cho_solve(
                (C, True), B, overwrite_b=True, check_finite=False
            )
...
def build_auxiliary_Coulomb_local_bas_wo_robust_fitting(
    ...
    for i in range(group_begin, group_end):
        ...
        for j in range(0, ngroup):
            ...
            W[
                aux_row_loc : aux_row_loc + naux_bra,
                aux_col_loc : aux_col_loc + naux_ket,
            ] = lib.ddot(
                V[:naux_bra, grid_shift : grid_shift + ngrid_now], aux_bas_ket.T
            )
        ...
        W[aux_row_loc : aux_row_loc + naux_bra, aux_col_loc:] = V[
            :naux_bra, grid_shift:
        ]
...
def build_auxiliary_Coulomb_local_bas(mydf, debug=True, use_mpi=False):
    ...
    for i in range(group_begin, group_end):
        ...
        sys.stdout.flush()
        mydf.W[:, aux_begin:aux_end] = lib.ddot(
            mydf.V_R[:, grid_shift : grid_shift + ngrid_now],
            mydf.aux_basis[i - group_begin].T,
        )
        grid_shift += ngrid_now
...
class PBC_ISDF_Info_Quad(ISDF.PBC_ISDF_Info):
    ...
    def build_partition_aoR(self, Ls=None):
        ...
        for i in range(self.cell.natm):
            tmp = ISDF_Local_Utils.AtmConnectionInfo(
                self.cell, i, self.distance_matrix, precision, rcut, rcut_max, atm2_bas
            )
        ...
        #                                                           self.use_mpi)
        # else:
        self.partition = ISDF_Local_Utils.get_partition(
            self.cell,
            self.coords,
            self.AtmConnectionInfo,
            Ls,
        ...
        for x in range(self.natm):
            ...
            logger.debug4(
                self, "len of partition[%d] = %d" % (x, len(self.partition[x]))
            )

        if self.use_aft_ao:
            self.aoR = ISDF_Local_Utils.get_aoR_analytic(
                self.cell,
                self.coords,
                self.partition,
                None,
    ...
    def allocate_k_buffer(self, nset=1):
        ...
        if self.direct:
            if hasattr(self, "build_k_buf") and self.build_k_buf is not None:
                if (
                    hasattr(self, "build_VW_in_k_buf")
                    and self.build_VW_in_k_buf is not None
                ):
                    allocated = True
        else:
            if hasattr(self, "build_k_buf") and self.build_k_buf is not None:
        ...
        if allocated:
            ...
            if self.direct:
                ...
                build_K_bunchsize = max(
                    maxsize_group_naux * self.naux // ngrid + 2, build_K_bunchsize
                )
                build_K_bunchsize = max(
                    maxsize_group_naux * max_nIP_involved // max_ngrid_involved + 2,
                    build_K_bunchsize,
                )
                self._build_K_bunchsize = build_K_bunchsize
                ...
                self.build_VW_in_k_buf = np.zeros(
                    (size0 + size1 + size2,), dtype=np.float64
                )
                ...
                log.info(
                    "In allocate_k_buffer, Density_RgAO_buf  memory = %d"
                    % (self.Density_RgAO_buf.nbytes)
                )
                log.info(
                    "In allocate_k_buffer, build_VW_in_k_buf memory = %d"
                    % (self.build_VW_in_k_buf.nbytes)
                )
                log.info(
                    "In allocate_k_buffer, build_k_buf       memory = %d"
                    % (self.build_k_buf.nbytes)
                )

            else:

                self.Density_RgAO_buf = np.zeros(
                    (self.naux, self.nao), dtype=np.float64
                )
                ...
                size0 = (
                    self.naux * max_nIP_involved
                    + self.naux * max_nao_involved
                    + self.naux * max(max_nIP_involved, max_nao_involved)
                )
                ...
                if self.with_robust_fitting:
                    size2 += (
                        self.naux * max_ngrid_involved + self.naux * max_nao_involved
                    )
                    size2 += self.naux * max_ngrid_involved
                self.build_k_buf = np.zeros(
                    (max(size0, size1, size2)), dtype=np.float64
                )
    ...
    def _construct_build_aoRg(self, IP_group, group=None):
        ...
        for atm_id in atm_ordering:
            ...
            aoR_holders_res[atm_id] = ISDF_Local_Utils.aoR_Holder(
                aoR, ao_involved, IP_ID_NOW, IP_ID_NOW + nIP, IP_ID_NOW, IP_ID_NOW + nIP
            )
    ...
    def build_IP_local(
        self, c=None, m=5, first_natm=None, group=None, Ls=None, debug=True
    ):
        ...
        for i in range(len(group)):
            ...
            self.partition_group_to_gridID[i] = np.array(
                self.partition_group_to_gridID[i], dtype=np.int32
            )
        ...
        self.grid_ID_ordered = ISDF_Local_Utils._get_grid_ordering(
            self.partition, self.group, self.use_mpi
        )
    ...
    def get_coulG(self):
        if hasattr(self, "rsjk") and self.rsjk is not None:
            ...
            coulG_bench = tools.get_coulG(
                self.cell_rsjk, mesh=self.cell_rsjk.mesh, omega=0.0
            )
    ...
    def build_auxiliary_Coulomb(self, debug=True):
        ...
        if self.V_W_cutoff is not None and self.V_W_cutoff > distance_max:
            logger.warn(
                self,
                "WARNING : V_W_cutoff is larger than the maximum distance in the cell",
            )
            self.V_W_cutoff = None  # no cutoff indeed
        if self.V_W_cutoff is not None:
            logger.debug4(
                self,
                "PBC_ISDF_Info_Quad:->build_auxiliary_Coulomb: V_W_cutoff   = %12.6e"
                % self.V_W_cutoff,
            )
            logger.debug4(
                self,
                "PBC_ISDF_Info_Quad:->build_auxiliary_Coulomb: distance_max = %12.6e"
                % distance_max,
            )
        ...
        if self.with_robust_fitting:
            ...
            build_auxiliary_Coulomb_local_bas_wo_robust_fitting(
                self, debug=debug, use_mpi=self.use_mpi
            )
    ...
    def check_aoR(self):
        for aoR_holder in self.aoR:
            max_abs_index = np.unravel_index(
                np.argmax(np.abs(aoR_holder.aoR)), aoR_holder.aoR.shape
            )
            ...
            print(
                ...
                " with indx = ",
                ao_indx,
                max_abs_index[1] + aoR_holder.global_gridID_begin,
            )
...
if __name__ == "__main__":
    ...
    basis = "gth-dzvp"
    pseudo = "gth-pade"
    prim_cell = build_supercell(
        atm, prim_a, Ls=[1, 1, 1], ke_cutoff=KE_CUTOFF, basis=basis, pseudo=pseudo
    )
    ...
    pbc_isdf_info = PBC_ISDF_Info_Quad(
        cell,
        with_robust_fitting=True,
        aoR_cutoff=1e-8,
        # direct=False,
