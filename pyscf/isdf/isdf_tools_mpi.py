    ...
    if size_of_recvbuf >= INT_MAX:
        ...
        for p0, p1 in prange(0, numpy.max(counts), blk_size_small):
            ...
            for i in range(comm_size):
                ...
                recvbuf[begin:end] = recvbuf_small[
                    i * blk_size_small : i * blk_size_small + counts_seg[i]
                ]
...
def reduce(sendbuf, op=MPI.SUM, root=0):
    ...
    for p0, p1 in lib.prange(0, sendbuf.size, BLKSIZE):
        comm.Reduce([send_seg[p0:p1], dtype], [recv_seg[p0:p1], dtype], op, root)
...
def scatter(sendbuf, root=0):
    ...
    for p0, p1 in prange(comm, 0, numpy.max(counts), BLKSIZE):
        counts_seg = _segment_counts(counts, p0, p1)
        comm.Scatterv(
            [sendbuf, counts_seg, displs + p0, mpi_dtype],
            [recvbuf[p0:p1], mpi_dtype],
            root,
        )
    ...
    if size_of_sendbuf >= INT_MAX:
        ...
        for p0, p1 in prange(0, max_counts, blk_size_small):
            ...
            for i in range(comm_size):
                ...
                sendbuf_small[
                    i * blk_size_small : i * blk_size_small + scounts_seg[i]
                ] = sendbuf[begin:end]

            comm.Alltoallv(
                [sendbuf_small, scounts_seg, sdispls_small, mpi_dtype],
            ...
            for i in range(comm_size):
                ...
                recvbuf[begin:end] = recvbuf_small[
                    i * blk_size_small : i * blk_size_small + rcounts_seg[i]
                ]
        ...
        for p0, p1 in prange(0, max_counts, BLKSIZE):
            ...
            rcounts_seg = _segment_counts(rcounts, p0, p1)
            comm.Alltoallv(
                [sendbuf, scounts_seg, sdispls + p0, mpi_dtype],
