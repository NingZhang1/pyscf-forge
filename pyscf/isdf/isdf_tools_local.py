...
class aoR_Holder:
    def __init__(
        ...
        assert (local_gridID_end - local_gridID_begin) == (
...
def _get_aoR_holders_memory(aoR_holders: list[aoR_Holder]):
    ...
    return sum(
        [_aoR_holder.size() for _aoR_holder in aoR_holders if _aoR_holder is not None]
    )
...
def flatten_aoR_holder(aoR_holders: list[aoR_Holder]):
    ...
    for _aoR_holder in aoR_holders:
        ...
        res_int.extend(
            [
                _aoR_holder.local_gridID_begin,
                _aoR_holder.local_gridID_end,
                _aoR_holder.global_gridID_begin,
                _aoR_holder.global_gridID_end,
            ]
        )
    ...
    for i in range(n):
        for j in range(i + 1, n):
            distance_matrix[i][j] = _distance_translation(
                cell.atom_coord(i), cell.atom_coord(j), a
            )
...
def get_partition(
    cell: Cell,
    coords,
    AtmConnectionInfoList: list[AtmConnectionInfo],
    Ls=[3, 3, 3],
    with_translation_symmetry=False,
    kmesh=None,
    use_mpi=False,
):  # by default split the cell into 4x4x4 supercell
    ...
    if use_mpi:
        from pyscf.isdf.isdf_tools_mpi import (
            rank,
            comm,
            comm_size,
            allgather,
    ...
    Ls_box = [
        lattice_vector[0] / mesh[0] * mesh_box[0],
        lattice_vector[1] / mesh[1] * mesh_box[1],
        lattice_vector[2] / mesh[2] * mesh_box[2],
    ]
    ...
    grid_id_global = np.arange(mesh[0] * mesh[1] * mesh[2], dtype=np.int32).reshape(
    ...
    for ix in range(nbox[0]):
        for iy in range(nbox[1]):
            for iz in range(nbox[2]):
                ...
                grid_ID = grid_id_global[
                    mesh_x_begin:mesh_x_end,
                    mesh_y_begin:mesh_y_end,
                    mesh_z_begin:mesh_z_end,
                ].flatten()
    ...
    if use_mpi == False:
        ...
        if ngroup % comm_size == 0:
            ...
            if rank < a:
                ...
                return a * ngroup_local + (rank - a) * (
                    ngroup_local - 1
                ), a * ngroup_local + (rank - a + 1) * (ngroup_local - 1)
                ...
                if i == 0:
                    ...
                    res = np.vstack(
                        (
                            res,
                            np.array(
                                [i * ngroup_local, (i + 1) * ngroup_local],
                                dtype=np.int32,
                            ),
                        )
                    )
                    ...
                    if i == a:
                        ...
                        res = np.vstack(
                            (
                                ...
                                np.array(
                                    [
                                        a * ngroup_local + (i - a) * (ngroup_local - 1),
                                        a * ngroup_local
                                        + (i - a + 1) * (ngroup_local - 1),
                                    ],
                                    dtype=np.int32,
                                ),
                            )
...
def _sync_aoR(aoR_holders, natm):
    ...
    for i in range(natm):
        if aoR_holders[i] is not None:
            ...
            grid_ID_begin.append(
                np.asarray([aoR_holders[i].global_gridID_begin], dtype=np.int32)
            )
        else:
            aoR.append(None)
            bas_id.append(None)
...
def get_aoR(
    ...
    AtmConnectionInfoList: list[AtmConnectionInfo] = None,
    distributed=False,
    use_mpi=False,
    sync_res=False,
):
    ...
    for atm_id in range(atm_begin, atm_end):
        ...
        if first_natm != cell.natm:
            ...
            if first_npartition == len(partition):
                atm_involved = []
                for atm_id_other, distance in AtmConnectionInfoList[
                    atm_id
                ].atm_connected_info:
    ...
    for atm_id in range(len(aoR_unpacked)):
        ...
        aoR_holder.append(
            aoR_Holder(
                aoR_holder_tmp,
                bas_id,
                grid_begin_unpacked[atm_id],
                grid_end_unpacked[atm_id],
                grid_begin_unpacked[atm_id],
                grid_end_unpacked[atm_id],
            )
        )
...
if __name__ == "__main__":
    ...
    basis = {
        "Cu1": "gth-dzvp-molopt-sr",
        "Cu2": "gth-dzvp-molopt-sr",
        "O1": "gth-dzvp-molopt-sr",
        "Ca": "gth-dzvp-molopt-sr",
    }
    pseudo = {
        "Cu1": "gth-pbe-q19",
        "Cu2": "gth-pbe-q19",
