...
def _select_IP_direct(
    ...
    if use_mpi:
        ...
        if rank == 0:
            logger.debug4(
                mydf, "_select_IP_direct: num_threads = %d", lib.num_threads()
            )
    else:
        rank = 0
    ...
    for p0, p1 in lib.prange(0, 1, 1):
        ...
        for atm_id in range(first_natm):
            ...
            aoR_atm = (
                ISDF_eval_gto(mydf.cell, coords=coords[grid_ID], out=aoR_atm) * weight
            )
            ...
            if aoR_cutoff is not None:
                logger.debug4(
                    mydf, "_select_IP_direct: aoR_cutoff = %12.6e", aoR_cutoff
                )
            ...
            aoPairBuffer = np.ndarray(
                (naux_now * naux_now, grid_ID.shape[0]),
                dtype=np.float64,
                buffer=buf_tmp,
                offset=offset,
            )
            ...
            global_buffer = np.ndarray(
                (1, grid_ID.shape[0]), dtype=np.float64, buffer=buf_tmp, offset=offset
            )
            ...
            logger.debug4(
                mydf,
                "_select_IP_direct: ngrid = %d, npt_find = %d, cutoff = %12.6e",
                grid_ID.shape[0],
                npt_find,
                cutoff,
            )
    ...
    if global_IP_selection and rank == 0:
        ...
        aoRg1 = np.ndarray(
            (naux_now, len(results)), dtype=np.float64, buffer=buf_tmp, offset=offset
        )
        ...
        fn_ik_jk_ijk(
            aoRg1.ctypes.data_as(ctypes.c_void_p),
        ...
        global_buffer = np.ndarray(
            (1, len(results)), dtype=np.float64, buffer=buf_tmp, offset=offset
        )
...
def build_aux_basis(mydf, debug=True, use_mpi=False):
    ...
    if not use_mpi or (use_mpi and rank == 0):
        A = np.asarray(
            lib.ddot(aoRg.T, aoRg, c=buffer1), order="C"
        )  # buffer 1 size = naux * naux
        square_inPlace(A)
    ...
    mydf.aux_basis = np.asarray(
        lib.ddot(aoRg.T, aoR), order="C"
...
class PBC_ISDF_Info(df.fft.FFTDF):
    ...
    def __init__(
        ...
        if aoR is not None:
            ...
            logger.debug4(
                self, "PBC_ISDF_Info: coords.shape[0] = %d", coords_now.shape[0]
            )
            ...
            if get_partition and aoR is None:
                for p0, p1 in lib.prange(0, coords_now.shape[0], bunchsize):
                    ...
                    AoR_Buf = ISDF_eval_gto(
                        self.cell, coords=coords_now[p0:p1], out=AoR_Buf
                    )
    ...
    def _allocate_jk_buffer(self, datatype):
        ...
        if self.jk_buffer is None:
            ...
            logger.debug4(
                self,
                "_allocate_jk_buffer: nao = %d, ngrids = %d, naux = %d",
                nao,
                ngrids,
                naux,
            )
            ...
            if hasattr(self, "IO_buf"):
                ...
                if self.IO_buf.size < (max(buffersize_k, buffersize_j) + size_ddot_buf):
                    self.IO_buf = np.zeros(
                        (max(buffersize_k, buffersize_j) + size_ddot_buf,),
                        dtype=datatype,
                    )

                self.jk_buffer = np.ndarray(
                    (max(buffersize_k, buffersize_j),),
                    dtype=datatype,
                    buffer=self.IO_buf,
                    offset=0,
                )
                ...
                self.ddot_buf = np.ndarray(
                    (nThreadsOMP, max((nao * nao) + 2, ngrids)),
                    dtype=datatype,
                    buffer=self.IO_buf,
                    offset=offset,
                )
                ...
                self.jk_buffer = np.ndarray(
                    (max(buffersize_k, buffersize_j),), dtype=datatype
                )
                self.ddot_buf = np.zeros(
                    (nThreadsOMP, max((nao * nao) + 2, ngrids)), dtype=datatype
                )
    ...
    def check_AOPairError(self):
        ...
        for i in range(nao):
            ...
            coeff = numpy.einsum("k,jk->jk", aoRg[i, :], aoRg).reshape(
                -1, self.IP_ID.shape[0]
            )
...
if __name__ == "__main__":
    ...
    pbc_isdf_info.build_IP_Sandeep(
        build_global_basis=True, c=C, global_IP_selection=False
    )
    ...
    for i in range(cell.natm):
        print(
            ...
            i,
            "partition = ",
            mf.with_df.partition[mf.with_df.partition == i].shape[0],
        )
    ...
    _pbc_isdf_info.build_IP_Sandeep(
        build_global_basis=True, c=12, global_IP_selection=False
    )
