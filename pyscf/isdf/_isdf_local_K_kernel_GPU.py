...
assert BACKEND.USE_TORCH_GPU == 1
...
def _build_V_local_bas_kernel(
    ...
    p1,
    # buffer #
    buffer,  # buffer on GPU
    buffer_fft,
    ### some helper info ###
    group_grid_ID,
    grid_ordering,  # should be put on GPU
    mesh,
    coulG_real,  # = coulG[:, :, mesh[2]//2+1].reshape(-1)
    # extra info #
    buffer_int,  # buffer for indx on GPU
    stream1,
    stream2,
):
    ngrids = np.prod(mesh)
    ...
    CLEAN(basis_buf)
    aux_basis_shape = aux_basis[group_id][p0:p1].shape
    aux_basis_buf = buffer.malloc(
        aux_basis_shape, dtype=FLOAT64, name="aux_basis_buf_gpu"
    )
    group_grid_ID_buf = buffer_int.malloc(
        group_grid_ID[group_id].shape, dtype=INT64, name="group_grid_ID_buf"
    )

    event1 = torch.cuda.Event()
    event2 = torch.cuda.Event()
    with torch.cuda.stream(stream1):
        aux_basis_buf.copy_(ToTENSOR(aux_basis[group_id][p0:p1]))
        event1.record(stream1)
    with torch.cuda.stream(stream2):
        group_grid_ID_buf.copy_(ToTENSOR(group_grid_ID[group_id]))
        event2.record(stream2)
    event1.synchronize()
    event2.synchronize()

    INDEX_COPY(basis_buf, 1, group_grid_ID_buf, aux_basis_buf)
...
def _build_W_local_bas_kernel(
    V: TENSORTy,
    aux_basis: list[TENSORTy],
    W: TENSORTy,
    # extra info #
    aux_basis_buf1,
    aux_basis_buf2,
    ...
    aux_ket_loc = 0
    # first data transfer #
    aux_1 = MALLOC(aux_basis[0].shape, dtype=FLOAT64, buf=aux_basis_buf1)
    aux_2 = None
    aux_1.copy_(aux_basis[0])
    # do the calculation #
    for i in range(len(aux_basis)):
        ...
        with torch.cuda.stream(stream2):
            ...
            aux_ket_loc += nket
        # copy the next data #
        if i + 1 < len(aux_basis):
            with torch.cuda.stream(stream1):
                if i % 2 == 0:
                    aux_2 = MALLOC(
                        aux_basis[i + 1].shape, dtype=FLOAT64, buf=aux_basis_buf2
                    )
                    aux_2.copy_(aux_basis[i + 1])
                else:
                    aux_1 = MALLOC(
                        aux_basis[i + 1].shape, dtype=FLOAT64, buf=aux_basis_buf1
                    )
                    aux_1.copy_(aux_basis[i + 1])
...
def _get_dm_RgR(
    ...
    ngrids = 0
    # first data transfer #
    aoR_1 = MALLOC(aoR_holders[0].aoR.shape, dtype=FLOAT64, buf=aoR_buf1)
    aoR_2 = None
    aoR_1.copy_(aoR_holders[0].aoR)
    # do the calculation #
    for i in range(len(aoR_holders)):
        _aoR_holder = aoR_holders[i]
        # do the calculation #
        if i % 2 == 0:
            aoR = aoR_1
        else:
        ...
        if nao_involved == nao:
            ...
            with torch.cuda.stream(stream2):
                tmp = buffer.malloc(
                    (nIP_involved, nao_involved), dtype=FLOAT64, name="tmp"
                )
                ao_involved_gpu = buffer_int.malloc(
                    (_aoR_holder.ao_involved.shape),
                    dtype=INT64,
                    name="ao_involved_gpu",
                )
                ao_involved_gpu.copy_(_aoR_holder.ao_involved)
                TAKE(dm_RgAO, ao_involved_gpu, 1, out=tmp)
                DOT(
                    tmp,
                    aoR,
                    c=dm_RgR[:, gridID_begin : gridID_begin + ngrids_involved],
                ...
                buffer.free(count=2)
        # copy the next data #
        if i + 1 < len(aoR_holders):
            with torch.cuda.stream(stream1):
                if i % 2 == 0:
                    aoR_2 = MALLOC(
                        aoR_holders[i + 1].aoR.shape, dtype=FLOAT64, buf=aoR_buf2
                    )
                    aoR_2.copy_(aoR_holders[i + 1].aoR)
                else:
                    aoR_1 = MALLOC(
                        aoR_holders[i + 1].aoR.shape, dtype=FLOAT64, buf=aoR_buf1
                    )
                    aoR_1.copy_(aoR_holders[i + 1].aoR)
        torch.cuda.synchronize()
...
def _get_dm_RgR_k(
    ...
    for ix, iy, iz in product(range(kmesh[0]), range(kmesh[1]), range(kmesh[2])):
        ...
        for i in range(len(aoR_holders)):
            ...
            ngrids_involved = aoR.shape[1]
            ngrids += ngrids_involved
            ao_involved = permutation[i]
            with torch.cuda.stream(stream2):
                tmp = buffer.malloc(
                    (nIP_involved, nao_involved), dtype=FLOAT64, name="tmp"
                )
                ao_involved_gpu = buffer.malloc(
                    (ao_involved.shape),
                    dtype=INT64,
                    name="ao_involved_gpu",
                )
                ao_involved_gpu.copy_(ao_involved)
                TAKE(dm_RgAO, ao_involved_gpu, 1, out=tmp)
                DOT(
                    tmp,
                    aoR,
    ...
    for i in range(len(aoR_holders)):
        ...
        if nao_involved == nao:
            ...
            with torch.cuda.stream(stream2):
                ...
                INDEX_ADD(half_K, 1, ao_involved_gpu, tmp)
                buffer.free(count=2)
        # copy the next data #
        if i + 1 < len(aoR_holders):
            with torch.cuda.stream(stream1):
    ...
    for ix, iy, iz in product(range(kmesh[0]), range(kmesh[1]), range(kmesh[2])):
        ...
        for i in range(len(aoR_holders)):
            ...
            with torch.cuda.stream(stream2):
                ...
                ao_involved_gpu.copy_(permutation[i])
                DOT(
                    dm_RgR[:, gridID_begin : gridID_begin + ngrids_involved],
                    aoR.T,
                    c=tmp,
...
def _final_contraction_k(
    K, aoRg_packed, p0, p1, half_K, box_2_segment, kmesh, buffer, buffer_int
):
    nao = half_K.shape[1]
    ...
    for ix, iy, iz in product(range(kmesh[0]), range(kmesh[1]), range(kmesh[2])):
        ...
        ao_involved_gpu = buffer_int.malloc(
            ao_involved.shape, dtype=INT64, name="ao_involved_buf"
        )
        ao_involved_gpu.copy_(ao_involved)
        aoRg_involved_gpu = buffer.malloc(
            aoRg_involved.shape, dtype=FLOAT64, name="aoRg_involved_buf"
        )
        aoRg_involved_gpu.copy_(aoRg_involved)

        ddot_res1 = buffer.malloc((loc_end - loc_begin, nao), dtype=FLOAT64)
        DOT(aoRg_involved_gpu, half_K, c=ddot_res1)
        ...
        if (loc_end - loc_begin) == naoPrim:
            ...
            CLEAN(half_K_2_add)
            INDEX_COPY(half_K_2_add, 0, ao_involved_gpu, ddot_res1)
