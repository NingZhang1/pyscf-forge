...
if __name__ == "__main__":
    ...
    for c in [15]:
        for N in [1]:
            ...
            myisdf = ISDF.PBC_ISDF_Info_Quad(
                cell, with_robust_fitting=True, aoR_cutoff=1e-8, direct=False
            )
            myisdf.verbose = 10
            myisdf.build_IP_local(
            ...
            mf_isdf.with_df.LS_THC_recompression(
                mf_isdf.with_df.aoRg_full()[0], force_LS_THC=False
            )
            isdf_pt = mp.RMP2(mf_isdf)
            isdf_pt.kernel()
            ...
            _myisdf = ISDF.PBC_ISDF_Info_Quad(
                cell,
            ...
            _myisdf.build_IP_local(
                c=15,
                m=5,
                group=group_partition,
                Ls=[Ls[0] * 10, Ls[1] * 10, Ls[2] * 10],
            )
