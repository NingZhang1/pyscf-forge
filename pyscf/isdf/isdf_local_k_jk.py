...
from pyscf.isdf.isdf_local_jk import (
    ...
    __get_DensityMatrixonRgAO_qradratic,
)
            ...
            for ix in range(kmesh[0]):
                for iy in range(kmesh[1]):
                    for iz in range(kmesh[2]):
                        ...
                        loc2 = (
                            (kmesh[0] - ix) % kmesh[0] * kmesh[1] * kmesh[2]
...
def _contract_j_dm_k_ls(mydf, _dm, use_mpi=False):
    ...
    max_nao_involved1 = np.max(
        [aoR_holder.aoR.shape[0] for aoR_holder in aoR1 if aoR_holder is not None]
    )
    ...
    max_ngrid_involved1 = np.max(
        [aoR_holder.aoR.shape[1] for aoR_holder in aoR1 if aoR_holder is not None]
    )
    ...
    if not use_mpi or (use_mpi and rank == 0):
        ...
        if in_real_space:
            ...
            J_complex = np.ndarray(
                (nao_prim, nao_prim * ncell_complex), dtype=np.complex128
            )
            fft_buf = np.ndarray(
                (nao_prim, nao_prim * ncell_complex), dtype=np.complex128
            )
            J_real = np.ndarray(
                (nao_prim, nao_prim * ncell), dtype=np.float64, buffer=J_complex
            )
...
def _get_k_kSym_robust_fitting_fast(mydf, _dm):
    ...
    DM_complex = np.ndarray(
        (nao_prim, nao_prim * ncell_complex),
        dtype=np.complex128,
    ...
    DM_RgRg_complex = np.ndarray(
        (nIP_prim, nIP_prim * ncell_complex),
        dtype=np.complex128,
        buffer=mydf.jk_buffer,
        offset=offset,
    )
    DM_RgRg_real = np.ndarray(
        (nIP_prim, nIP_prim * ncell), dtype=np.float64, buffer=DM_RgRg_complex
    )
    ...
    buf_A = np.ndarray(
        (nao_prim, nao_prim), dtype=np.complex128, buffer=mydf.jk_buffer, offset=offset
    )

    offset2 = offset + (nao_prim * nao_prim) * buf_A.itemsize
    buf_B = np.ndarray(
        (nao_prim, nIP_prim), dtype=np.complex128, buffer=mydf.jk_buffer, offset=offset2
    )

    offset3 = offset2 + (nao_prim * nIP_prim) * buf_B.itemsize
    buf_C = np.ndarray(
        (nao_prim, nIP_prim), dtype=np.complex128, buffer=mydf.jk_buffer, offset=offset3
    )

    offset4 = offset3 + (nao_prim * nIP_prim) * buf_C.itemsize
    buf_D = np.ndarray(
        (nIP_prim, nIP_prim), dtype=np.complex128, buffer=mydf.jk_buffer, offset=offset4
    )
    ...
    K_complex_buf = np.ndarray(
        (nao_prim, nao_prim * ncell_complex),
        dtype=np.complex128,
        buffer=mydf.jk_buffer,
        offset=offset,
    )
    K_real_buf = np.ndarray(
        (nao_prim, nao_prim * ncell),
        dtype=np.float64,
        buffer=mydf.jk_buffer,
        offset=offset,
    )
    ...
    DM_RgR_complex = np.ndarray(
        (nIP_prim, nGridPrim * ncell_complex),
        dtype=np.complex128,
        buffer=mydf.jk_buffer,
        offset=offset_after_dm,
    )
    DM_RgR_real = np.ndarray(
        (nIP_prim, nGridPrim * ncell), dtype=np.float64, buffer=DM_RgR_complex
    )
    ...
    K_real_buf = np.ndarray(
        (nao_prim, nao_prim * ncell), dtype=np.float64, buffer=K_complex_buf
    )
    ...
    if not in_real_space:
        ...
        K_real = np.ndarray(
            (nao_prim, nao_prim * ncell), dtype=np.float64, buffer=K_complex
        )
...
def _get_k_kSym(mydf, _dm):
    ...
    DM_real = np.ndarray(
        (nao_prim, nao), dtype=np.float64, buffer=mydf.jk_buffer, offset=offset
    )
    DM_real.ravel()[:] = dm[:nao_prim, :].ravel()[:]
...
def _get_k_kSym_direct(mydf, _dm, use_mpi=False):
    ...
    from pyscf.isdf._isdf_local_K_direct import (
        reset_profile_buildK_time,
        add_cputime_RgAO,
        add_walltime_RgAO,
        log_profile_buildK_time,
    )
    ...
    if (use_mpi and rank == 0) or not use_mpi:
        ...
        for iset in range(nset):
            Ktmp = _RowCol_FFT_bench(
                K[iset, :nao_prim, :], kmesh, inv=True, TransBra=False, TransKet=True
            )
            K_res.append(Ktmp)
...
def get_jk_dm_translation_symmetry(
    mydf,
    dm,
    hermi=1,
    kpt=np.zeros(3),
    ...
    **kwargs
    ...
    if exxdiv == "ewald":
        if np.allclose(kpt, np.zeros(3)):
            ...
            logger.warn(
                mydf, "get_jk_dm_k_quadratic: Exxdiv for k-point is not supported"
            )
...
def _get_k_kSym_direct_mimic_MPI(mydf, _dm, use_mpi=False):
    ...
    for _rank_ in range(COMM_SIZE):
        ...
        for group_id, atm_ids in enumerate(group):
            ...
            Density_RgAO_tmp = np.ndarray(
                (nset, naux_tmp, nao), buffer=Density_RgAO_buf
            )
            offset_density_RgAO_buf = (
                Density_RgAO_tmp.size * Density_RgAO_buf.dtype.itemsize
            )
