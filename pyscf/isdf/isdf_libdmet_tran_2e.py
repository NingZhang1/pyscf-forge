...
def get_emb_eri_isdf(mydf, C_ao_emb: np.ndarray = None, symmetry=4):
    ...
    eri = np.zeros(
        (nspin * (nspin + 1) // 2, nemb**2, nemb**2), dtype=np.float64
    )  ## the ordering of spin is aa, bb, ab
                ...
                for ix in range(kmesh[0]):
                    for iy in range(kmesh[1]):
                        for iz in range(kmesh[2]):
                            ...
                            if with_robust_fitting:
                                aoR_tmp[
                                    ILOC_ * nao_prim : (ILOC_ + 1) * nao_prim, :
                                ] = aoR_fullcol[
                                    ILOC * nao_prim : (ILOC + 1) * nao_prim, :
                                ]
                            aoRg_tmp[ILOC_ * nao_prim : (ILOC_ + 1) * nao_prim, :] = (
                                aoRg_fullcol[ILOC * nao_prim : (ILOC + 1) * nao_prim, :]
                            )
            ...
            for ky in range(kmesh[1]):
                for kz in range(kmesh[2]):
                    ...
                    for i in range(nspin):
                        ...
                        _emb_pair = np.einsum(
                            "iP,jP->ijP", _emb_R[i][ILOC], _emb_R[i][ILOC]
                        )
                        ...
                        for ix in range(kmesh[0]):
                            for iy in range(kmesh[1]):
                                for iz in range(kmesh[2]):
                                    ...
                                    ILOC_ = (
                                        ix_ * kmesh[1] * kmesh[2] + iy_ * kmesh[2] + iz_
                                    )
                                    ILOC = ix * kmesh[1] * kmesh[2] + iy * kmesh[2] + iz
                                    ...
                                    lib.ddot(
                                        a=V[
                                            :,
                                            ILOC_
                                            * ngrid_per_box : (ILOC_ + 1)
                                            * ngrid_per_box,
                                        ],
                                        b=_emb_pair.T,
                                        alpha=1.0,
                                        c=tmp_V[i][
                                            ILOC * nIP_prim : (ILOC + 1) * nIP_prim, :
                                        ],
                                        beta=1.0,
                                    )
            ...
            for iy in range(kmesh[1]):
                for iz in range(kmesh[2]):
                    ...
                    if nspin == 1:
                        ...
                        emb_pair_Rg = np.einsum(
                            "iP,jP->ijP", emb_Rg[0][ILOC], emb_Rg[0][ILOC]
                        )
                        ...
                        if nspin == 2:
                            ...
                            emb_pair_Rg_alpha = np.einsum(
                                "iP,jP->ijP", emb_Rg[0][ILOC], emb_Rg[0][ILOC]
                            )
                            emb_pair_Rg_beta = np.einsum(
                                "iP,jP->ijP", emb_Rg[1][ILOC], emb_Rg[1][ILOC]
                            )
...
class aoR_Holder2(aoR_Holder):
    def __init__(
        self, aoR, ao_involved, grid_involved, global_gridID_begin, global_gridID_end
    ):
        super(aoR_Holder2, self).__init__(
            aoR,
            ao_involved,
            0,
...
def _get_embR_box(
    ...
    ddot_buf = np.ndarray(
        (nemb, max_grid), dtype=np.float64, buffer=buffer, offset=offset_now
    )
    offset_now += ddot_buf.size * ddot_buf.itemsize
    pack_buf = np.ndarray(
        (nemb, max_nao), dtype=np.float64, buffer=buffer, offset=offset_now
    )
...
def get_emb_eri_isdf_fast(
            ...
            for kz in range(kmesh[2]):
                if with_robust_fitting:
                    ...
                    for i in range(nspin):
                        ...
                        log.info(
                            "emb values on grids box %2d %2d %2d for spin %1d, nao : %4d/%4d, grid points %6d/%6d"
                            % (
                                kx,
                                ky,
                                kz,
                ...
                for i in range(nspin):
                    ...
                    log.info(
                        "emb values on IPs   box %2d %2d %2d for spin %1d, nao : %4d/%4d, IPs         %6d/%6d"
                        % (
                            kx,
                            ky,
                            kz,
    ...
    log.info(
        "max_nemb_involved = %4d, max_ngrid_invovled = %4d"
        % (max_nemb_involved, max_ngrid_invovled)
    )
    ...
    for i in range(nspin):
        if with_robust_fitting:
            not_invovled_box.append(
                [
                    _id_
                    for (_id_, holder) in enumerate(emb_R[i])
                    if holder.aoR.shape[0] == 0 or holder.aoR.shape[1] == 0
                ]
            )
        else:
            not_invovled_box.append(
                [
                    _id_
                    for (_id_, holder) in enumerate(emb_Rg[i])
                    if holder.aoR.shape[0] == 0 or holder.aoR.shape[1] == 0
                ]
            )
        if len(not_invovled_box[-1]) > 0:
            log.info(
                "Warning: some boxes are not involved in the calculation for %s"
                % (not_invovled_box)
            )
    ...
    if with_robust_fitting:
        buffer_tmp_embpair_on_grids = np.zeros(
            (nnemb, max_ngrid_invovled), dtype=np.float64
        )
        buffer_packVW = np.zeros((loop_bunchsize, max_ngrid_invovled), dtype=np.float64)
    else:
        buffer_tmp_embpair_on_grids = np.zeros(
            (nnemb, max_nIP_involved), dtype=np.float64
        )
            ...
            for iy in range(kmesh[1]):
                for iz in range(kmesh[2]):
                    for j in range(len(group)):
                        ...
                        if not skip:
                            W_[p0:p1, aux_ket_shift : aux_ket_shift + naux_ket] = (
                                lib.ddot(
                                    V_[:, grid_shift : grid_shift + ngrid_now],
                                    aux_basis_ket.T,
                                )
                            )
    ...
    eri = np.zeros(
        (nspin * (nspin + 1) // 2, nnemb, nnemb), dtype=np.float64
    )  ## the ordering of spin is aa, bb, ab
    ...
    for group_id, atm_ids in enumerate(group):
        ...
        for p0, p1 in lib.prange(0, naux_tmp, loop_bunchsize):
            if direct:
                V_tmp = np.ndarray(
                    (p1 - p0, np.prod(mesh)), dtype=np.float64, buffer=buffer_V_tmp
                )
...
if __name__ == "__main__":
    ...
    prim_cell = build_supercell(
        atm, prim_a, Ls=[1, 1, 1], basis=basis, ke_cutoff=KE_CUTOFF
    )
    prim_mesh = prim_cell.mesh
    ...
    pbc_isdf_info2 = PBC_ISDF_Info_Quad(
        supercell,
        with_robust_fitting=True,
        aoR_cutoff=1e-8,
        direct=False,
    ...
    for i in range(eri_emb.shape[0]):
        for j in range(eri_emb.shape[1]):
            print(
                eri_emb[i, j],
                eri_emb_benchmark[i, j],
                eri_emb[i, j] / eri_emb_benchmark[i, j],
            )
