...
for kmesh in kmeshes:
    ...
    f = os.path.join(lib.param.TMPDIR, "tmp.h5")

    isdf_old = ISDF_Local(
        cell, with_robust_fitting=True, limited_memory=True, build_V_K_bunchsize=56
    )
    isdf_old._isdf_to_save = f
    isdf_old.build(c=30, m=5, rela_cutoff=1e-4, group=group)
    # isdf.force_translation_symmetry(kmesh)

    isdf_new = ISDF_Local(cell)
    isdf_new._isdf = f
    isdf_new.build(c=30, m=5, rela_cutoff=1e-4, group=group)
