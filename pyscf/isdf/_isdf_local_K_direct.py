...
def log_profile_buildK_time(mydf, use_mpi=False):
    ...
    if not use_mpi:
        log.info(
            "In _isdf_get_K_direct_kernel RgAO  cputime = %16.3f walltime = %16.3f paralell = %4.2f"
            % (cputime_RgAO, walltime_RgAO, cputime_RgAO / walltime_RgAO)
        )
        log.info(
            "In _isdf_get_K_direct_kernel RgR   cputime = %16.3f walltime = %16.3f paralell = %4.2f"
            % (cputime_RgR, walltime_RgR, cputime_RgR / walltime_RgR)
        )
        log.info(
            "In _isdf_get_K_direct_kernel V     cputime = %16.3f walltime = %16.3f paralell = %4.2f"
            % (cputime_V, walltime_V, cputime_V / walltime_V)
        )
        log.info(
            "In _isdf_get_K_direct_kernel W     cputime = %16.3f walltime = %16.3f paralell = %4.2f"
            % (cputime_W, walltime_W, cputime_W / walltime_W)
        )
        log.info(
            "In _isdf_get_K_direct_kernel Ktmp1 cputime = %16.3f walltime = %16.3f paralell = %4.2f"
            % (cputime_Ktmp1, walltime_Ktmp1, cputime_Ktmp1 / walltime_Ktmp1)
        )
        log.info(
            "In _isdf_get_K_direct_kernel Ktmp2 cputime = %16.3f walltime = %16.3f paralell = %4.2f"
            % (cputime_Ktmp2, walltime_Ktmp2, cputime_Ktmp2 / walltime_Ktmp2)
        )
    else:
...
def _add_kpnt_info(mydf):
    if hasattr(mydf, "kmesh"):
        assert mydf.kmesh is None or (
            mydf.kmesh[0] == 1 and mydf.kmesh[1] == 1 and mydf.kmesh[2] == 1
        )
...
def _isdf_get_K_direct_kernel_1(
        ...
        for ky in range(kmesh[1]):
            for kz in range(kmesh[2]):
                ...
                for atm_id in group[group_id]:
                    ...
                    IP_2_atm_id.extend(
                        [atm_id + ILOC * natm_prim]
                        * aoRg1[atm_id + ILOC * natm_prim].aoR.shape[1]
                    )
    ...
    for p0, p1 in lib.prange(begin_id, end_id, bunchsize):
        ...
        if construct_K1:
            Density_RgR_tmp = np.ndarray(
                (p1 - p0, ngrid), buffer=Density_RgR_buf, offset=0, dtype=np.float64
            )
        else:
            Density_RgR_tmp = np.ndarray(
                (p1 - p0, naux), buffer=Density_RgR_buf, offset=0, dtype=np.float64
            )
            ...
            for ky in range(kmesh[1]):
                for kz in range(kmesh[2]):
                    ...
                    if kx != 0 or ky != 0 or kz != 0:
                        if construct_K1:
                            col_permutation = mydf._get_permutation_column_aoR(
                                kx, ky, kz
                            )
                        else:
                            col_permutation = mydf._get_permutation_column_aoRg(
                                kx, ky, kz
                            )
                    ...
                    for atm_id in atm_ordering[:natm_prim]:
                        ...
                        if use_cutoff:
                            if distance_cutoff is not None:
                                distance = np.min(
                                    distance_matrix[
                                        unique_elements, ILOC * natm_prim + atm_id
                                    ]
                                )
                        ...
                        if (nao_involved == nao) and (kx == 0 and ky == 0 and kz == 0):
                            ...
                            Density_RgAO_packed = np.ndarray(
                                (p1 - p0, nao_involved),
                                buffer=dm_RgAO_buf,
                        ...
                        if construct_K1:
                            grid_begin = (
                                aoR_holder.global_gridID_begin + ILOC * ngrid_prim
                            )
                        else:
                            grid_begin = (
                                aoR_holder.global_gridID_begin + ILOC * nIP_prim
                            )

                        ddot_res_RgR = np.ndarray(
                            (p1 - p0, ngrid_now), buffer=ddot_res_RgR_buf
                        )
                        lib.ddot(Density_RgAO_packed, aoR_holder.aoR, c=ddot_res_RgR)
                        Density_RgR_tmp[:, grid_begin : grid_begin + ngrid_now] = (
                            ddot_res_RgR
                        )
            ...
            for ky in range(kmesh[1]):
                for kz in range(kmesh[2]):
                    ...
                    for atm_id in atm_ordering[:natm_prim]:
                        ...
                        nao_involved = aoR_holder.aoR.shape[0]
                        ddot_res = np.ndarray(
                            (p1 - p0, nao_involved), buffer=K1_tmp1_ddot_res_buf
                        )
                            ...
                            if distance_cutoff is None:
                                V2_tmp_max2 = np.max(
                                    np.abs(
                                        V2_tmp[
                                            :,
                                            grid_loc_begin : grid_loc_begin + ngrid_now,
                                        ]
                                    )
                                )
                        ...
                        lib.ddot(
                            V2_tmp[:, grid_loc_begin : grid_loc_begin + ngrid_now],
                            aoR_holder.aoR.T,
                            c=ddot_res,
                        )
                ...
                for iy in range(kmesh[1]):
                    for iz in range(kmesh[2]):
                        ...
                        if use_cutoff:
                            if distance_cutoff is not None:
                                distance = np.min(
                                    distance_matrix[
                                        unique_elements,
                                        ILOC * natm_prim : (ILOC + 1) * natm_prim,
                                    ]
                                )
                                if distance > distance_cutoff:
                                    skip = True
                        ...
                        for j in range(len(group)):
                            ...
                            if not skip:
                                W_tmp[
                                    p0:p1, aux_ket_shift : aux_ket_shift + naux_ket
                                ] = lib.ddot(
                                    V_tmp[:, grid_shift : grid_shift + ngrid_now],
                                    aux_basis_ket.T,
                                )
