...
def select_IP_local_ls_k_drive(
    ...
    mydf.IP_flat = _expand_primlist_2_superlist(
        mydf.IP_flat_prim, mydf.kmesh, mydf.mesh
    )
    ...
    if build_aoR_FFT:
        ...
        aoRg_Tmp = (
            ISDF_eval_gto(
                mydf.cell, coords=coords[mydf.IP_flat], shls_slice=(0, nbas_prim)
            )
            * weight
        )

        mydf.aoRg_FFT = np.zeros(
            (nao_prim, ncell_complex * mydf.nIP_Prim), dtype=np.complex128
        )
        mydf.aoRg_FFT_real = np.ndarray(
            (nao_prim, np.prod(kmesh) * mydf.nIP_Prim),
            dtype=np.double,
            buffer=mydf.aoRg_FFT,
            offset=0,
        )
        ...
        for i in range(ncell_complex):
            aoRg_packed.append(
                mydf.aoRg_FFT[:, i * nIP_Prim : (i + 1) * nIP_Prim].copy()
            )
        del mydf.aoRg_FFT
    ...
    if mydf.with_robust_fitting and build_aoR_FFT:
        ...
        aoR_tmp = (
            ISDF_eval_gto(
                mydf.cell,
                coords=coords[mydf.grid_ID_ordered],
                shls_slice=(0, nbas_prim),
            )
            * weight
        )
        mydf.aoR_FFT = np.zeros(
            (nao_prim, ncell_complex * ngrids_prim), dtype=np.complex128
        )
        mydf.aoR_FFT_real = np.ndarray(
            (nao_prim, np.prod(kmesh) * ngrids_prim),
            dtype=np.double,
            buffer=mydf.aoR_FFT,
            offset=0,
        )
        ...
        for i in range(ncell_complex):
            aoR_packed.append(
                mydf.aoR_FFT[:, i * ngrids_prim : (i + 1) * ngrids_prim].copy()
            )
        del mydf.aoR_FFT
            ...
            for iy in range(kmesh[1]):
                for iz in range(kmesh[2]):
                    for j in range(ngroup):
                        ...
                        mydf.W[
                            aux_bra_shift : aux_bra_shift + naux_bra,
                            aux_ket_shift : aux_ket_shift + naux_ket,
                        ] = lib.ddot(
                            V[
                                aux_bra_shift : aux_bra_shift + naux_bra,
                                grid_shift : grid_shift + ngrid_now,
                            ],
                            aux_basis_ket.T,
                        )
...
class PBC_ISDF_Info_Quad_K(ISDF_Local.PBC_ISDF_Info_Quad):
    ...
    def build_partition_aoR(self, Ls=None):
        ...
        assert (
            len(self.partition_prim) == self.natmPrim
        )  ## the grid id is the global grid id

        self.partition = _expand_partition_prim(
            self.partition_prim, self.kmesh, self.mesh
        )
        ...
        self.partition_activated_id = np.array(
            self.partition_activated_id, dtype=np.int32
        )
    ...
    def build_IP_local(self, c=5, m=5, group=None, Ls=None, debug=True):
        ...
        self.grid_ID_ordered = _get_grid_ordering_k(
            self.partition_group_to_gridID, self.kmesh, self.mesh
        )
        self.grid_ID_ordered_prim = self.grid_ID_ordered[
            : ngrids // np.prod(self.kmesh)
        ].copy()
        self.partition_group_to_gridID = _expand_partition_prim(
            self.partition_group_to_gridID, self.kmesh, self.mesh
        )
        ...
        for i in range(len(self.grid_ID_ordered_prim)):
            ...
            self.grid_ID_ordered_prim[i] = (
                ix * self.meshPrim[1] * self.meshPrim[2] + iy * self.meshPrim[2] + iz
            )
    ...
    def _allocate_jk_buffer(self, dtype=np.float64):
        ...
        if self.outcore is False:
            ...
            if hasattr(self, "IO_buf"):
                ...
                self.jk_buffer = np.ndarray(
                    (size_buf), dtype=np.float64, buffer=self.IO_buf, offset=0
                )
                self.ddot_buf = np.ndarray(
                    (size_ddot_buf),
                    dtype=np.float64,
                    buffer=self.IO_buf,
                    offset=size_buf,
        ...
        if self.aoR_col_permutation[loc] is None:
            ...
            for aoR_holder in self.aoR:
                ...
                for ao_id in ao_involved:
                    ...
                    nao_id_new = (
                        box_x_new * self.kmesh[1] * self.kmesh[2] * self.nao_prim
                        + box_y_new * self.kmesh[2] * self.nao_prim
                        + box_z_new * self.nao_prim
                        + nao_id
                    )
            ...
            for ix in range(self.kmesh[0]):
                for iy in range(self.kmesh[1]):
                    for iz in range(self.kmesh[2]):
                        ...
                        loc_ = (
                            ix_ * self.kmesh[1] * self.kmesh[2]
                            + iy_ * self.kmesh[2]
                            + iz_
                        )
                        for i in range(
                            loc_ * self.natmPrim, (loc_ + 1) * self.natmPrim
                        ):
    ...
    def get_jk(
        ...
        if is_single_kpt:
            ...
            vj, vk = get_jk_dm_translation_symmetry(
                self, dm, hermi, kpts[0], kpts_band, with_j, with_k, exxdiv=exxdiv
            )
        else:

            ### first construct J and K ###

            from pyscf.isdf.isdf_local_k_jk import (
                _contract_j_dm_k_ls,
                _get_k_kSym_robust_fitting_fast,
                _get_k_kSym,
                _get_k_kSym_direct,
                _get_k_kSym_direct_mimic_MPI,
            )
...
if __name__ == "__main__":
    ...
    pbc_isdf_info = PBC_ISDF_Info_Quad_K(
        prim_cell,
        kmesh=Ls,
        with_robust_fitting=True,
        aoR_cutoff=1e-8,
    ...
    pbc_isdf_info.build_IP_local(
        c=C, m=5, group=prim_partition, Ls=[Ls[0] * 10, Ls[1] * 10, Ls[2] * 10]
    )
    pbc_isdf_info.verbose = 10

    weight = np.sqrt(cell.vol / pbc_isdf_info.coords.shape[0])
    aoR_benchmark = (
        ISDF_eval_gto(cell, coords=pbc_isdf_info.coords[pbc_isdf_info.grid_ID_ordered])
        * weight
    )
        ...
        for iy in range(Ls[1]):
            for iz in range(Ls[2]):
                ...
                for _loc_, data in enumerate(pbc_isdf_info.aoR):
                    aoR_unpacked[perm_col[_loc_], ngrid : ngrid + data.aoR.shape[1]] = (
                        data.aoR
                    )
    ...
    pbc_isdf_info = ISDF_Local.PBC_ISDF_Info_Quad(
        cell,
        with_robust_fitting=True,
        aoR_cutoff=1e-8,
        direct=True,
    ...
    pbc_isdf_info.build_IP_local(
        c=C, m=5, group=group_partition, Ls=[Ls[0] * 10, Ls[1] * 10, Ls[2] * 10]
    )
    ...
    aoR_benchmark = (
        ISDF_eval_gto(cell, coords=pbc_isdf_info.coords[grid_ordered]) * weight
    )
    diff = aoR_benchmark - aoR_unpacked
    print("diff = ", np.linalg.norm(diff) / np.sqrt(aoR_unpacked.size))
    # exit(1)
